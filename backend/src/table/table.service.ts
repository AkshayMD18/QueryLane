...
export class TableService {
  ...
  async getColumns(tableName: string) {
    const tableMetadata = await this.tableRepository.findOne({
      where: { tableName },
    });
    if (!tableMetadata) {
    ...
    const group = await this.groupsService.getGroupById(tableMetadata.groupId);
    const { rawColumns } = await this.tableRepo.fetchTableDetails(
  ...
  async getTableData(tableName: string, page?: number, limit?: number) {
    ...
    const group = await this.groupsService.getGroupById(tableMetadata.groupId);
    const { data, total } = await this.tableRepo.getTableData(
      group?.databasePath ?? 'app.sqlite',
      tableMetadata.tableName,
      currentPage,
      currentLimit,
  ...
  private async createDynamicTable(
    ...
    await this.tableRepo.createDynamicTable(
      databasePath,
      tableName,
      columnDefs,
    );
  ...
  private async insertDataBatch(
    ...
    for (let i = 0; i < data.length; i += batchSize) {
      ...
      await this.tableRepo.insertDataBatch(
        databasePath,
        tableName,
        colNames,
        rowsSql,
