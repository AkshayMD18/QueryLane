...
export class QueryController {
  ...
  async executeQuery(@Body() executeQueryDto: ExecuteQueryDto) {
    ...
    const result = await this.queryService.executeAndStoreQuery(
      query,
      tableId,
      userQuery,
    );
  ...
  async executeGroupQuery(@Body() executeGroupQueryDto: ExecuteGroupQueryDto) {
    ...
    const result = await this.queryService.executeAndStoreGroupQuery(
      query,
      groupId,
      userQuery,
    );
  ...
  async getAllQueriesForGroup(@Query() getGroupQueriesDto: GetGroupQueriesDto) {
    ...
    const results = await Promise.all(
      queries.map(async (q: any) => {
        const data = await this.queryService.executeGroupQuery(
          q.query,
          allowedTables,
          groupId,
        );
