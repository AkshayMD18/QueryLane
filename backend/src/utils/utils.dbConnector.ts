...
export class PostgresDbConnector {
    async createSnapshot(
        ...
        const excluded = new Set(excludedTables.map((table) => {
        ...
        try {
            ...
            for (const table of tableResult.rows) {
                ...
                const columns = await this.getColumns(client, schemaName, table.table_name);
    ...
    private async getColumns(client: Client, schemaName: string, tableName: string) {
    ...
    private async readRows(client: Client, schemaName: string, tableName: string) {
        const result = await client.query(
            `SELECT *
    ...
    private normalizeType(type: string): string {
        ...
        if (['numeric', 'decimal', 'real', 'double precision'].includes(type)) return 'decimal';
