...
export function validateSelectQuery(
  ...
  try {
    ...
    if (tableName) {
      ...
      const referencedTables = tableList.map((t) =>
      ...
      if (unauthorizedTables.length > 0) {
        throw new BadRequestException(
          `Unauthorized table references: ${unauthorizedTables.join(', ')}`,
        );
      }
    }
    ...
    if (columns && columns.length > 0) {
      ...
      if (unauthorizedColumns.size > 0) {
        throw new BadRequestException(
          `Unauthorized or non-existent columns referenced: ${Array.from(unauthorizedColumns).join(', ')}`,
        );
      }
    }
  } catch (err: any) {
...
export function validateGroupSelectQuery(
  query: string,
